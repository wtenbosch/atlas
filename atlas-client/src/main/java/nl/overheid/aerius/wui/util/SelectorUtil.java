...
public final class SelectorUtil {
  ...
  static {
    APPLICATION_TYPES.add(UglyBoilerPlate.DEFAULT_TYPE_YEAR);
  }
  ...
  static boolean isMatch(final Collection<Selector> criteria, final Collection<Selector> selections) {
    // If the set of options contain anything not in the selections, there cannot be
    // a match
  ...
  /**
   *
   * @param challenger Candidate that is challenging the status-quo
   * @param old        The status-quo that is being challenged
   * @param selections The criteria which define the victor
