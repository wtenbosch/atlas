...
EventPreview, HasAnimation, HasCloseHandlers<PopupPanel> {
  ...
  public static enum AnimationType {
    CENTER, ONE_WAY_CORNER, ROLL_DOWN, ROLL_RIGHT
  }
  ...
  static class ResizeAnimation extends Animation {
     ...
     */
    public ResizeAnimation(final PopupPanel panel) {
      this.curPanel = panel;
    }
     ...
     */
    public void setState(final boolean showing, final boolean isUnloading) {
      ...
      // EDIT AERIUS: ROLL_DOWN is allowed to animate while hiding.
      if (curPanel.animType != AnimationType.CENTER && curPanel.animType != AnimationType.ROLL_DOWN && curPanel.animType != AnimationType.ROLL_RIGHT && !showing) {
        animate = false;
      }
    ...
    protected void onComplete() {
      ...
      // AERIUS FIX: We don't want to clip this because we're showing elements outside the popup panel (css box-shadow, for example, and an optional arrow)
    ...
    protected void onUpdate(double progress) {
      ...
      int right = 0;
      int bottom = 0;
      final int height = (int) (progress * offsetHeight);
      final int width = (int) (progress * offsetWidth);
     ...
     */
    private String getRectString(final int top, final int right, final int bottom, final int left) {
  ...
  private final ResizeHandler glassResizer = new ResizeHandler() {
    ...
    public void onResize(final ResizeEvent event) {
      ...
      final int winWidth = Window.getClientWidth();
      final int winHeight = Window.getClientHeight();
      ...
      final int height = Document.get().getScrollHeight();
   ...
   */
  public PopupPanel(final boolean autoHide, final boolean modal) {
    this(autoHide);
    this.modal = modal;
  }
   ...
   */
  public void center() {
    final boolean initiallyShowing = showing;
    final boolean initiallyAnimated = isAnimationEnabled;
    ...
    final int left = Window.getClientWidth() - getOffsetWidth() >> 1;
    final int top = Window.getClientHeight() - getOffsetHeight() >> 1;
   ...
   */
  @Deprecated
  public boolean onKeyPressPreview(final char key, final int modifiers) {
    return true;
  }
   ...
   */
  @Deprecated
  public boolean onKeyUpPreview(final char key, final int modifiers) {
    return true;
  }
   ...
   */
  public void removeAutoHidePartner(final Element partner) {
    assert partner != null : "partner cannot be null";
    if (autoHidePartners != null) {
   ...
   */
  public void setAutoHideOnHistoryEventsEnabled(final boolean enabled) {
   ...
   */
  public void setPreviewingAllNativeEvents(final boolean previewAllNativeEvents) {
    this.previewAllNativeEvents = previewAllNativeEvents;
  }
 
  @Override
  public void setTitle(final String title) {
    final Element containerElement = getContainerElement();
  ...
  void maybeUpdateSize() {
    ...
    final Widget w = super.getWidget();
    if (w != null) {
   ...
   */
  void setAnimation(final ResizeAnimation animation) {
   ...
   */
  private boolean eventTargetsPartner(final NativeEvent event) {
    if (autoHidePartners == null) {
      return false;
    }
    ...
    if (Element.is(target)) {
      for (final Element elem : autoHidePartners) {
   ...
   */
  private void position(final UIObject relativeObject, final int offsetWidth,
    ...
    if (LocaleInfo.getCurrentLocale().isRTL()) { // RTL case
      ...
      if (offsetWidthDiff > 0) {
        ...
        final int textBoxLeftValForRightEdge = textBoxAbsoluteLeft
            + textBoxOffsetWidth;
    ...
    // the text box
    final int distanceToWindowBottom = windowBottom
  ...
  private void previewNativeEvent(final NativePreviewEvent event) {
    ...
    final boolean eventTargetsPopupOrPartner = eventTargetsPopup(nativeEvent)
    ...
    case Event.ONFOCUS: {
      final Element target = nativeEvent.getTarget();
