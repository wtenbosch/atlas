...
/**
 * A PropertySource computes the binding properties to use for calling the GWT
 * compiler.
 * 
 * @constructor
 * @param {string}
 ...
 * @param {Function}
 *            propertyProvidersHolder - a function returning property providers
 *            and their values when invoked.
 * @param {MetaTagParser}
 *            metaTagParser - an instance of {@link MetaTagParser}
 */
...
/**
 * Setup the environment that property providers need to run and initialize
 * property providers with that environment.
 * 
 * @returns an object containing property providers and possible values.
 */
PropertySource.prototype.__getProvidersAndValues = function(
		propertyProvidersHolder) {
	...
	// This function is defined in recompile_template.js and parameters being
	// passed here need to be
	// kept in sync.
	return propertyProvidersHolder(__gwt_getMetaProperty,
 ...
 * 
 * @param {string}
 *            propName - the binding property name
...
function Dialog() {
  ...
  dialog.style.boxShadow = "0 0 2px #08639B";
  ...
  var dialogText = $doc.createElement('div');
  dialogText.style.color = '#ccc';
  dialogText.style.wordWrap = 'break-word';
  dialogText.style.padding = '1em';
  dialogText.style.textAlign = 'center';
  
  this.__dialogTextContainer = dialogText;
 ...
 * 
 * @param {external:Node}
 *            node - the node to add to the dialog
 */
Dialog.prototype.add = function(node) {
	this.__dialogTextContainer.appendChild(node);
};
...
Dialog.prototype.clear = function() {
	while (this.__dialogTextContainer.firstChild) {
		this.__dialogTextContainer.removeChild(this.__dialogTextContainer.firstChild);
	}
};
 ...
 *            moduleName - the name of the gwt module
 * @param {Object}
 *            permutationProperties - A map of binding property names (string)
 *            to their values (string).
 * @returns
 */
 ...
 * 
 * @param {Function}
 *            finishCallback - the callback to invoke once compile finishes
 */
...
Recompiler.prototype.getCodeServerBaseUrl = function() {
	...
	for (var i = 0; i < scriptTagsToSearch.length; i++) {
		var candidate = StringHelper
				.removeQueryString(scriptTagsToSearch[i].src);
		if (StringHelper.endsWith(candidate, expectedSuffix)) {
			return this.__getBaseUrl(candidate);
		}
	}
	throw "unable to find the script tag that includes /recompile-requester/"
			+ moduleName;
};
...
Recompiler.prototype.loadApp = function() {
	var url = this.getCodeServerBaseUrl() + this.__moduleName + '/'
			+ this.__moduleName + '.nocache.js';
...
/**
 * BaseUrlProvider provides the url to the original server that the app has been
 * loaded from. This is not the url of super dev mode.
 * 
 * @constructor
 * @param {string}
 *            moduleName - the module for which we should determine the base
 *            url.
 */
...
BaseUrlProvider.prototype.getBaseUrl = function() {
	// This code follows the order in
